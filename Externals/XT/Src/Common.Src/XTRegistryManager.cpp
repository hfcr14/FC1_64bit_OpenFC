...
HKEY CXTRegistryManager::GetAppRegistryKey()
{
	...
	ASSERT(m_pWinApp->m_pszProfileName != NULL);
	...
	{
		DWORD dw;
		if (RegCreateKeyEx(hSoftKey, m_pWinApp->m_pszRegistryKey, 0, REG_NONE,
			REG_OPTION_NON_VOLATILE, KEY_WRITE|KEY_READ, NULL,
			&hCompanyKey, &dw) == ERROR_SUCCESS)
		{
			RegCreateKeyEx(hCompanyKey, m_pWinApp->m_pszProfileName, 0, REG_NONE,
				REG_OPTION_NON_VOLATILE, KEY_WRITE|KEY_READ, NULL,
				&hAppKey, &dw);
		}
	}
...
BOOL CXTRegistryManager::WriteProfileInt(LPCTSTR lpszSection, LPCTSTR lpszEntry, int nValue)
{
	...
	{
		// If this asserts, you need to call SetRegistryKey(...); in your CWinApp::InitInstance()
		// method.  Calling this will initialize the registry key m_pszRegistryKey.  If you want
		// to write to an INI file instead of the system registry, call CXTRegistryManager::SetINIFileName(...)
		// prior to making this call.  If you want to always write to an INI file by default
		// without having to call CXTRegistryManager::SetINIFileName(...) each time, you can set 
		// xtAfxData.strINIFileName equal to your INI file path.
		ASSERT(m_pWinApp->m_pszRegistryKey != NULL);

		// m_pszRegistryKey not defined, return FALSE.
		if (m_pWinApp->m_pszRegistryKey == NULL)
			return FALSE;
...
UINT CXTRegistryManager::GetProfileInt(LPCTSTR lpszSection, LPCTSTR lpszEntry, int nDefault)
{
	...
	if (m_strINIFileName.IsEmpty()) // use registry
	{
		...
		// m_pszRegistryKey not defined, return nDefault.
		if (m_pWinApp->m_pszRegistryKey == NULL)
			return nDefault;
...
CString CXTRegistryManager::GetProfileString(LPCTSTR lpszSection, LPCTSTR lpszEntry, LPCTSTR lpszDefault)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		// m_pszRegistryKey not defined, return lpszDefault.
		if (m_pWinApp->m_pszRegistryKey == NULL)
			return lpszDefault;
									...
									CStringArray * arrayNames)
{
	...
	while (ERROR_NO_MORE_ITEMS != RegEnumValue(hKey, index++, strValue.GetBuffer(255), &dwLen,
		NULL, &dwType, NULL, NULL))
	{
		strValue.ReleaseBuffer();

		if(mapItems) mapItems->SetAt(strValue, dwType);
		if (arrayNames) arrayNames->Add(strValue);

		// this value was reset to the actual length, thank you very much.
		dwLen = 255;
	}
...
int CXTRegistryManager::EnumKeys(LPCTSTR lpszSection, CStringArray & arrayKeys)
{
	...
	while (ERROR_NO_MORE_ITEMS != RegEnumKeyEx(hKey, index++, strValue.GetBuffer(255), &dwLen,
		NULL, NULL, NULL, NULL))
	{
		strValue.ReleaseBuffer();
		arrayKeys.Add(strValue);
