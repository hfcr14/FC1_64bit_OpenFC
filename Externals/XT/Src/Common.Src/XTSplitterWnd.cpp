...
{
	COLORREF clr3DFace = CXTColorRef::GetColor(XPCOLOR_SPLITTER_FACE);
	COLORREF clr3DShadow = CXTColorRef::GetColor(XPCOLOR_3DSHADOW);
	...
	case splitBorder:
		if (m_bFlatSplit)
		{
			if (!(m_dwxStyle & XT_SPLIT_NOBORDER))
			{
				...
				pDC->Draw3dRect(rect, clr3DShadow, xtAfxData.clr3DHilight);
			} else
			{
				pDC->Draw3dRect(rect, clr3DFace, clr3DFace);
				rect.InflateRect(-CX_BORDER, -CY_BORDER);
		...
		else
		{
			pDC->Draw3dRect(rect, clr3DShadow, xtAfxData.clr3DHilight);
			rect.InflateRect(-CX_BORDER, -CY_BORDER);
			pDC->Draw3dRect(rect, xtAfxData.clrWindowFrame, clr3DFace);
		}
		return;

	case splitIntersection:
		ASSERT(!CXTOSVersionInfo::Get().IsWin95OrGreater());
		break;
	...
	case splitBox:
		...
		{
			pDC->Draw3dRect(rect, clr3DFace, xtAfxData.clrWindowFrame);
			rect.InflateRect(-CX_BORDER, -CY_BORDER);
			pDC->Draw3dRect(rect, xtAfxData.clr3DHilight, clr3DShadow);
			rect.InflateRect(-CX_BORDER, -CY_BORDER);
		}
		break;
...
void CXTSplitterWndEx::OnDrawSplitter(CDC* pDC, ESplitType nType, const CRect& rectArg)
{
	...
	case splitBorder:
		return;
	case splitBox:
		pDC->Draw3dRect(rect, xtAfxData.clr3DFace, xtAfxData.clrWindowFrame);
		rect.InflateRect(-CX_BORDER, -CY_BORDER);
	...
	COLORREF clr = xtAfxData.clr3DFace;
	pDC->FillSolidRect(rect, clr);
}
...
AFX_STATIC void AFXAPI _xtAfxLayoutRowCol(CSplitterWnd::CRowColInfo* pInfoArray,
...
AFX_STATIC void AFXAPI _xtAfxDeferClientPos(AFX_SIZEPARENTPARAMS* lpLayout,
...
void CXTSplitterWndEx::RecalcLayout()
{
	...
	_xtAfxLayoutRowCol(m_pColInfo, m_nCols, rectInside.Width(), m_cxSplitterGap);
	_xtAfxLayoutRowCol(m_pRowInfo, m_nRows, rectInside.Height(), m_cySplitterGap);
	...
	if (m_bHasHScroll && m_bHasVScroll)
	{
		...
		_xtAfxDeferClientPos(&layout, pScrollBar,
	...
	if (m_bHasHScroll)
	{
		...
		for (col = 0; col < m_nCols; col++)
		{
			...
			_xtAfxDeferClientPos(&layout, pScrollBar, x, y, cx, cy, TRUE);
	...
	//BLOCK: Reposition all the panes
	{
		...
		for (col = 0; col < m_nCols; col++)
		{
			...
			for (row = 0; row < m_nRows; row++)
			{
				...
				_xtAfxDeferClientPos(&layout, pWnd, x, y, cx, cy, FALSE);
				y += cy + m_cySplitterGap;
			}
			x += cx + m_cxSplitterGap;
		}
...
void CXTSplitterWndEx::OnPaint()
{
    ...
    if (m_bShowTopBorder)
	{
        ...
        CPen penHilight(PS_SOLID, 1, xtAfxData.clr3DHilight);
...
BOOL CXTSplitterWndEx::OnEraseBkgnd(CDC* pDC) 
{
	...
	pDC->FillSolidRect(rectClient, CXTColorRef::GetColor(XPCOLOR_SPLITTER_FACE));
