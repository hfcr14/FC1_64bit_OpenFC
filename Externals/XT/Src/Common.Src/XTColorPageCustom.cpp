...
XT_HAS_FOCUS CXTColorBase::m_eHasFocus = none;
...
void CXTColorWnd::DrawCrossHair(CDC* pDC)
{
	...
	CPen* pOldPen = pDC->SelectObject((m_eHasFocus == wnd) ? &pen1 : &pen2);
...
void CXTColorLum::DrawLuminanceBar(CDC *pDC)
{
	...
	if (xtAfxData.bWin95 || xtAfxData.bWin98)
	{
...
void CXTColorLum::DrawSliderArrow(CDC* pDC)
{
	...
	CPen pen2(PS_SOLID, 1, xtAfxData.clr3DShadow);

	CPen* pOldPen = pDC->SelectObject((m_eHasFocus == lum) ? &pen1 : &pen2);
...
IMPLEMENT_DYNCREATE(CXTColorPageCustom, CPropertyPage)
...
CXTColorPageCustom::CXTColorPageCustom(CXTColorDialog* pParentSheet) : CPropertyPage(CXTColorPageCustom::IDD)
{
...
void CXTColorPageCustom::OnPaint() 
{
	CPaintDC dc(this); // device context for painting
	CRect rc;
	GetClientRect(&rc);
...
BOOL CXTColorPageCustom::OnEraseBkgnd(CDC* pDC) 
{
	UNUSED_ALWAYS(pDC);
	return TRUE;
}
