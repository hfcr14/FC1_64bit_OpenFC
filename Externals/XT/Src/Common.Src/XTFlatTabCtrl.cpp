...
CXTFlatTabCtrl::CXTFlatTabCtrl()
{
	...
	m_nClientWidth	 = 0;
	m_nClientHeight  = 0;
	m_pNormFont 	 = &xtAfxData.font;
	m_pBoldFont 	 = &xtAfxData.fontBold;
...
BOOL CXTFlatTabCtrl::OnEraseBkgnd(CDC* pDC) 
{
	if (GetItemCount() == 0)
	{
		pDC->FillSolidRect(m_rectViews, xtAfxData.clr3DFace);
	}
	return TRUE;
}
...
void CXTFlatTabCtrl::OnPaint() 
{
	...
	if (rectClip.right >= rcArrows.right)
	{
		...
		tabDC.FillSolidRect(rcTabs, xtAfxData.clr3DFace);
...
void CXTFlatTabCtrl::DrawButton(CDC* pDC, CXTFTButtonState& button_state) const
{
	...
	{
		pDC->Draw3dRect(r, xtAfxData.clr3DShadow, xtAfxData.clr3DHilight);
	}
	else
	{
		pDC->Draw3dRect(r, xtAfxData.clr3DHilight, xtAfxData.clr3DShadow);
	}
	r.DeflateRect(1, 1);
	pDC->FillSolidRect(r, xtAfxData.clr3DFace);
...
{
	// set COLORREF values to default colors.
	...
	// set pen values to default colors.
	m_penOutline.DeleteObject();
	m_penOutline.CreatePen(PS_SOLID, 1, m_clrBtnText);
	
	m_penShadow.DeleteObject();
	m_penShadow.CreatePen(PS_SOLID, 1, m_clr3DShadow);
	
	m_penWindow.DeleteObject();
	m_penWindow.CreatePen(PS_SOLID, 1, m_clr3DHilight);
	
	m_penBackNonSel.DeleteObject();
	m_penBackNonSel.CreatePen(PS_SOLID, 1, m_clr3DFace);
	
	m_penBackSel.DeleteObject();
	m_penBackSel.CreatePen(PS_SOLID, 1, m_clrWindow);
