...
DWORD AFXAPI _xtRelease(LPUNKNOWN* lplpUnknown)
{
...
#ifndef _DEBUG
#ifndef RELEASE
#define RELEASE(lpUnk) _xtRelease((LPUNKNOWN*)&lpUnk)
#endif
#else

// generate larger but type safe code in debug build
...
void CXTHtmlView::Navigate2(LPITEMIDLIST pIDL, DWORD dwFlags /* = 0 */,
	LPCTSTR lpszTargetFrameName /* = NULL */)
{
	ASSERT(m_pBrowserApp != NULL);
	...
	COleVariant vPIDL;
	_xtAfxMakeOleVariant(vPIDL, pIDL);
#else
...
BOOL CXTHtmlView::OnEraseBkgnd(CDC* pDC) 
{
	if (m_bLoading == true)
	{
		...
		CXTMemDC memDC(pDC, r, xtAfxData.clrWindow);
		...
		if (m_bLoadingMessage)
		{
			...
			memDC.SetTextColor(xtAfxData.clrGrayText);
