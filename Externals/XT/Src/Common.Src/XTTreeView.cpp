// XTTreeView.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CXTTreeView diagnostics

#ifdef _DEBUG
void CXTTreeView::AssertValid() const
{
	CTreeView::AssertValid();
}

void CXTTreeView::Dump(CDumpContext& dc) const
{
	CTreeView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CXTTreeView message handlers

void CXTTreeView::OnLButtonDown(UINT nFlags, CPoint point) 
{
    ...
    if ( m_bMultiSelect )
    {
		...
		HTREEITEM hItemHit = GetTreeCtrl().HitTest(point, &nHitFlags);
		HTREEITEM hItemSel = GetTreeCtrl().GetSelectedItem();
...
void CXTTreeView::OnRButtonDown(UINT nFlags, CPoint point) 
{
    ...
    HWND hWnd = GetTreeCtrl().GetOwner()->m_hWnd;
    ...
    if (::IsWindow(hWnd))
	{
		...
		mHDR.code     = NM_RCLICK;
		mHDR.idFrom   = GetTreeCtrl().GetDlgCtrlID();
...
void CXTTreeView::OnSetFocus(CWnd* pOldWnd) 
{
	if (m_bMultiSelect)
	{
...
void CXTTreeView::OnKillFocus(CWnd* pNewWnd) 
{
...
void CXTTreeView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (!m_bMultiSelect) {
	...
	HTREEITEM hNext = bDir ? GetTreeCtrl().GetPrevVisibleItem(hSel) : GetTreeCtrl().GetNextVisibleItem(hSel);
	if (!hNext) {
		hNext = hSel;
	}
	if (bShift) {
...
BOOL CXTTreeView::OnItemExpanding(NMHDR* pNMHDR, LRESULT* pResult) 
{
	if (!m_bMultiSelect)
		return FALSE;
	...
	*pResult = 0;
	...
	if ((pNMTreeView->action == TVE_COLLAPSE) || (pNMTreeView->action == TVE_COLLAPSERESET))
	{
		...
		if (bWasFocus && !bWasSel) {
			GetTreeCtrl().SelectItem(NULL);	//stop parent from gaining selection
		}
	}
...
HTREEITEM CXTTreeView::GetNextItem( HTREEITEM hItem )
{
	HTREEITEM hti = NULL;
...
HTREEITEM CXTTreeView::GetPrevItem( HTREEITEM hItem )
{
	HTREEITEM hti = NULL;
...
HTREEITEM CXTTreeView::GetLastItem( HTREEITEM hItem )
{
	...
	while( GetTreeCtrl().ItemHasChildren( hItem ) != NULL )
	{
...
HTREEITEM CXTTreeView::FindItemInBranch(LPCTSTR lpszSearch, BOOL bCaseSensitive /*= FALSE*/, 
	BOOL bWholeWord /*= FALSE*/, HTREEITEM htiItem /*= NULL*/)
{
    HTREEITEM htiFound = NULL;

    if (!GetTreeCtrl().ItemHasChildren(htiItem))
        return NULL;
    ...
    HTREEITEM htiChild = GetTreeCtrl().GetChildItem(htiItem);
    while (htiChild != NULL)
    {
        if (GetTreeCtrl().ItemHasChildren(htiChild))
        {
		...
		CString strItemText = GetTreeCtrl().GetItemText( htiChild );
		if (!bCaseSensitive) {
        ...
        htiChild = GetTreeCtrl().GetNextSiblingItem(htiChild);
    }

    return NULL;
}

HTREEITEM CXTTreeView::FindItem(LPCTSTR lpszSearch, BOOL bCaseSensitive /*= FALSE*/, 
	BOOL bDownDir /*= TRUE*/, BOOL bWholeWord /*= FALSE*/, HTREEITEM hItem /*= NULL*/)
{
	...
	while( htiCur && (htiCur != htiSel || bFirstPass) )
	{
		...
		CString strItemText = GetTreeCtrl().GetItemText( htiCur );
		if (!bCaseSensitive) {
			strItemText.MakeLower();
		}
...
BOOL CXTTreeView::IsFindValid( HTREEITEM )
{
	return TRUE;
}

void CXTTreeView::SetItemFont(HTREEITEM hItem, LOGFONT& logfont)
{
	XT_CLRFONT cf;
...
BOOL CXTTreeView::GetItemFont(HTREEITEM hItem, LOGFONT * plogfont)
{
	XT_CLRFONT cf;
	if (!m_mapColorFont.Lookup(hItem, cf)) {
		return FALSE;
...
void CXTTreeView::SetItemBold(HTREEITEM hItem, BOOL bBold)
{
	SetItemState( hItem, bBold ? TVIS_BOLD: 0, TVIS_BOLD );
	InvalidateRect(NULL);
}

BOOL CXTTreeView::GetItemBold(HTREEITEM hItem)
{
	return GetItemState( hItem, TVIS_BOLD ) & TVIS_BOLD;
}

void CXTTreeView::SetItemColor(HTREEITEM hItem, COLORREF color)
{
	XT_CLRFONT cf;
	if (!m_mapColorFont.Lookup( hItem, cf)) {
...
COLORREF CXTTreeView::GetItemColor(HTREEITEM hItem)
{
	XT_CLRFONT cf;
	if (!m_mapColorFont.Lookup(hItem, cf)) {
		return (COLORREF)-1;
...
BOOL CXTTreeView::OnEraseBkgnd(CDC* pDC) 
{
	UNREFERENCED_PARAMETER(pDC);
	return FALSE;
}

void CXTTreeView::DoPaint()
{
	CClientDC dc(this);
	...
	CTreeView::DefWindowProc( WM_PAINT, (WPARAM)memDC.m_hDC, 0 );
	...
	int iVisibleCount = GetTreeCtrl().GetVisibleCount()+1;
	...
	while (hItem && iVisibleCount--)
	{
		...
		// draw the focus rect.
		if ( bFocused && !GetTreeCtrl().GetEditControl( ) )
		{
...
void CXTTreeView::OnPaint() 
{
	CPaintDC dc(this);
	DoPaint(); // perform custom paint.
}

HTREEITEM CXTTreeView::GetPrevSelectedItem(HTREEITEM hItem)
{
	for (hItem = GetTreeCtrl().GetPrevVisibleItem(hItem);
	     hItem != NULL;
		 hItem = GetTreeCtrl().GetPrevVisibleItem(hItem))
	{
...
UINT CXTTreeView::GetSelectedCount() const
{
	UINT nCount = 0;
	HTREEITEM hItem = GetFirstSelectedItem();
	while (hItem)
...
BOOL CXTTreeView::EnableMultiSelect(BOOL bMultiSelect)
{
...
BOOL CXTTreeView::SetItemState(HTREEITEM hItem, UINT nState, UINT nStateMask)
{
	ASSERT(hItem);
	...
	HTREEITEM hFocus  = GetTreeCtrl().GetSelectedItem();		      // current focus
	...
	if (nStateMask & TVIS_FOCUSED)
	{
		...
		if (nState & TVIS_FOCUSED)
		{
			...
			if (!bWasFocus && bFocusWasSel)
			{
				...
				GetTreeCtrl().SelectItem(NULL); //will cause notify, but can be taken as focus change
				GetTreeCtrl().SetItemState(hFocus, TVIS_SELECTED, TVIS_SELECTED);
				UpdateWindow();
			}

			if (!GetTreeCtrl().SelectItem(hItem)) { //set focus (will consequently select, if not already focused)
				return FALSE;
			}
		...
		else
		{
			...
			{
				//it had the focus
				GetTreeCtrl().SelectItem(NULL);	//clear focus
				if (!(nStateMask & TVIS_SELECTED))
				{
					//didn't want to affect select state
					if (bWasSel)
	...
	if (nSM & TVIS_SELECTED)
	{
		...
		item.hItem         = hItem;
		item.lParam        = GetTreeCtrl().GetItemData(hItem);
	
		if (SendNotify(&nmtv.hdr)) {
			return FALSE;	//sel-changing stopped
		}
		
		VERIFY( GetTreeCtrl().SetItemState(hItem, nS, nSM) );
	...
	return GetTreeCtrl().SetItemState(hItem, nS, nSM);
}

UINT CXTTreeView::GetItemState(HTREEITEM hItem, UINT nStateMask) const
{
	UINT n = GetTreeCtrl().GetItemState(hItem, nStateMask & ~TVIS_FOCUSED);
	if (nStateMask & TVIS_FOCUSED)
	{
...
BOOL CXTTreeView::SelectItem(HTREEITEM hItem)
{
	if (m_bMultiSelect)
	{
...
BOOL CXTTreeView::FocusItem(HTREEITEM hItem)
{
	...
	{
		hItem = CTreeView::GetTreeCtrl().GetSelectedItem();
		if (hItem) {
...
LRESULT CXTTreeView::SendNotify(LPNMHDR pNMHDR)
{
...
HTREEITEM CXTTreeView::GetFirstSelectedItem() const
{
...
HTREEITEM CXTTreeView::GetNextSelectedItem(HTREEITEM hItem) const
{
	hItem = GetTreeCtrl().GetNextVisibleItem(hItem);
	while (hItem)
	{
...
void CXTTreeView::SelectAll(BOOL bSelect /*= TRUE*/)
{
//	SetFocus();
...
void CXTTreeView::SelectAllIgnore(BOOL bSelect, HTREEITEM hIgnore)
{
...
void CXTTreeView::SelectItems(HTREEITEM hFirst, HTREEITEM hLast, BOOL bOnly /*= TRUE*/)
{
...
BOOL CXTTreeView::OnButtonDown(BOOL bLeft, UINT nFlags, CPoint point)
{
	UINT      nHF   = 0;
	...
	if (!bBase)
	{
		hItem = GetTreeCtrl().HitTest(point, &nHF);
		if (hItem)
		{
			...
			if (!bBase && bLeft && (GetStyle() & TVS_CHECKBOXES))
			{
				...
				if (bBase)
				{
						...
						while (h)
						{
							if (!GetCheck(h) == bChk) //! to ensure 0 or 1
							{
								...
								if (h != hItem) //clicked item will be done last
								{
									...
									nmtv.itemNew.lParam = GetTreeCtrl().GetItemData(h);
									SendNotify(&nmtv.hdr);
								}
#endif
							}
					...
					nmtv.itemNew.lParam = GetTreeCtrl().GetItemData(hItem);
					SendNotify(&nmtv.hdr);
					return TRUE;
				}
#endif
...
void CXTTreeView::DoPreSelection(HTREEITEM hItem, BOOL bLeft, UINT nFlags)
{
	if (bLeft)
	{
		...
		{
			if (!m_hSelect) {
				m_hSelect = GetTreeCtrl().GetSelectedItem();	//focus
			}
...
void CXTTreeView::DoAction(HTREEITEM hItem, BOOL bLeft, UINT nFlags, CPoint point)
{
    m_bActionDone = false;
	::SetCapture(m_hWnd);
...
void CXTTreeView::DoBanding(UINT nFlags, CPoint point)
{
	if (::GetFocus() != m_hWnd) {
		::SetFocus(m_hWnd);
	}
...
void CXTTreeView::UpdateSelectionForRect(LPCRECT pRect, UINT nFlags, CTypedPtrList<CPtrList, HTREEITEM>& list)
{
	CRect rect;
	BOOL bSel;
	POSITION pos;
	...
	while (hItem)
	{
		...
		GetTreeCtrl().GetItemRect(hItem, &rect, m_bBandLabel);
...
BOOL CXTTreeView::SelectChildren(HTREEITEM hParent, BOOL bSelect /*= TRUE*/, BOOL bRecurse /*= TRUE*/)
{
...
void CXTTreeView::GetSelectedList(CTypedPtrList<CPtrList, HTREEITEM>& list) const
{
	list.RemoveAll();
...
BOOL CXTTreeView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	case TTN_SHOW:
		{
			...
			HWND hWnd = TreeView_GetToolTips( GetTreeCtrl( ).m_hWnd );
			if ( hWnd != NULL )
			{
...
BOOL CXTTreeView::PreTranslateMessage(MSG* pMsg) 
{
	...
	CEdit* pEditCtrl = GetTreeCtrl().GetEditControl();
...
void CXTTreeView::OnDeleteItem(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_TREEVIEW* pNMTreeView = (NM_TREEVIEW*)pNMHDR;
