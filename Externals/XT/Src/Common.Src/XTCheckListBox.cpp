...
CXTCheckListState::CXTCheckListState(UINT uBitmapID)
{
	CBitmap bitmap;
...
CXTCheckListState::~CXTCheckListState()
{
...
void CXTCheckListBox::SetCheck(int nIndex, int nCheck)
{
	...
	if (lResult != LB_ERR)
	{
		...
		XT_CHECK_DATA* pState = (XT_CHECK_DATA*)lResult;
		
		if (pState == NULL)
			pState = new XT_CHECK_DATA;
...
void CXTCheckListBox::PreDrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	{
		XT_CHECK_DATA* pState = (XT_CHECK_DATA*)drawItem.itemData;
...
void CXTCheckListBox::PreMeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	if (GetStyle() & LBS_OWNERDRAWVARIABLE)
	{
		...
		{
			XT_CHECK_DATA* pState = (XT_CHECK_DATA*)measureItem.itemData;
...
int CXTCheckListBox::PreCompareItem(LPCOMPAREITEMSTRUCT lpCompareItemStruct)
{
	...
	{
		XT_CHECK_DATA* pState = (XT_CHECK_DATA*)compareItem.itemData1;
		compareItem.itemData1 = pState->m_dwUserData;
	}
	if (compareItem.itemData2 != 0 && compareItem.itemData2 != LB_ERR)
	{
		XT_CHECK_DATA* pState = (XT_CHECK_DATA*)compareItem.itemData2;
...
void CXTCheckListBox::PreDeleteItem(LPDELETEITEMSTRUCT lpDeleteItemStruct)
{
	...
	{
		XT_CHECK_DATA* pState = (XT_CHECK_DATA*)deleteItem.itemData;
...
LRESULT CXTCheckListBox::OnLBGetText(WPARAM wParam, LPARAM lParam)
{
	...
	if (lResult != LB_ERR)
	{
		XT_CHECK_DATA* pState = (XT_CHECK_DATA*)lParam;
