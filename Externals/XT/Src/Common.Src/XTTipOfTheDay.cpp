	...
	: CDialog(IDD, pParent)
{
	...
	m_strTipTitle.LoadString(XT_IDS_DIDYOUKNOW);
...
#ifdef UNICODE
	m_pStream = _wfopen((TCHAR*)(LPCTSTR)m_strTipFile, _T("r"));
#else
	m_pStream = fopen(m_strTipFile, _T("r"));
#endif
	if (m_pStream == NULL) {
		m_strTipText.LoadString(XT_IDS_FILE_ABSENT);
		return;
	} 
	...
	if (fseek(m_pStream, iFilePos, SEEK_SET) != 0) {
		AfxMessageBox(XT_IDS_FILE_CORRUPT);
	} else {
...
void CXTTipOfTheDay::OnPaint() 
{
	...
	// Draw a 3D border.
	dc.Draw3dRect(m_rcBorder, xtAfxData.clr3DShadow,
		xtAfxData.clrWindow);

	// Draw the shadow area color where the light bulb appears.
	dc.FillSolidRect(m_rcShadow, xtAfxData.clr3DShadow);

	// Draw the tip text background area.
	dc.FillSolidRect(m_rcHilite, xtAfxData.clrWindow);
...
BOOL CXTTipOfTheDay::OnInitDialog() 
{
	...
	m_ok.SetFont(&xtAfxData.font);
	m_showTips.SetFont(&xtAfxData.font);
	m_btnNextTip.SetFont(&xtAfxData.font);
