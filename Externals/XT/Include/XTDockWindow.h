...
protected:
    ...
    CXTBarButton* m_pBtnMinMax;       // Min/max button used in the caption area.
...
public:
    ...
    //			the following:
	//			[ul]
    //			[li]<b>CBRS_XT_CLOSEBTN</b> The control bar only has a close
	//			button when docked.[/li]
    //			[li]<b>CBRS_XT_MINMAXBTN</b> The control bar has a minimize
	//			button when docked.[/li]
    //			[li]<b>CBRS_XT_BUTTONS</b> The control bar has a minimize and close
	//			button when docked.[/li]
	//          [li]<b>CBRS_XT_NOFLOAT</b> Docking window cannot be floated from its 
	//          docked state.[/li]
    ...
    // Summary:	This member function creates a CXTDockWindow (docking window).
    ...
    // Summary:	This member function is called to set the normal size for 
    //			the control bar.
    ...
    // Summary:	This member function is called to create a CFrameWnd object used
...
protected:
	...
	virtual bool HideShowButtons(int nCmdShow);
...
AFX_INLINE bool CXTDockWindow::HasButtons() {
    return (HasCloseButton() || HasMinMaxButton());
}
AFX_INLINE bool CXTDockWindow::HasCloseButton() {
    return (HasGripper() && (m_bp.dwStyle & CBRS_XT_CLOSEBTN) == CBRS_XT_CLOSEBTN);
}
AFX_INLINE bool CXTDockWindow::HasMinMaxButton() {
    return (HasGripper() && (m_bp.dwStyle & CBRS_XT_MINMAXBTN) == CBRS_XT_MINMAXBTN);
}
