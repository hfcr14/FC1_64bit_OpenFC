...
public:
	...
	// Input:   bUseOfficeFont - True to use the "Tahoma" font if found on the operating system.
    // Summary: Call this member function to use the Office font (Tahoma) if it exists.
    void UseOfficeFont(bool b);

private:
	bool FontExists(CString& strFaceName);
	bool CreateSysFont(XT_LOGFONT& lf, CFont& font, long lfWeight=-1, char lfUnderline=-1, long lfOrientation=-1, long lfEscapement=-1);
};
...
//////////////////////////////////////////////////////////////////////
// Summary: CXTNoFlickerWnd is a TBase derived general purpose template helper
...
template<class TBase>
AFX_INLINE CXTNoFlickerWnd<TBase>::CXTNoFlickerWnd() {
	m_crBack = xtAfxData.clr3DFace;
}

template<class TBase>
AFX_INLINE void CXTNoFlickerWnd<TBase>::SetBackColor(COLORREF crBack) {
	m_crBack = crBack;
}

template<class TBase>
AFX_INLINE COLORREF CXTNoFlickerWnd<TBase>::GetBackColor() {
	return m_crBack; // An RGB value.
}

template<class TBase>
LRESULT CXTNoFlickerWnd<TBase>::WindowProc(UINT message,WPARAM wParam,LPARAM lParam)
{
	switch (message)
	{
	case WM_PAINT:
