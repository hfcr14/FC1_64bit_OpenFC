...
CLightScene::CLightScene()
	: m_uiCurrentPatchNumber(0), m_pCurrentPatch(NULL), m_pISerializationManager(NULL), m_uiStartMemoryConsumption(0)
{
...
CLightScene::~CLightScene()
{
	SAFE_RELEASE(m_pISerializationManager);
}
...
inline void CLightScene::MergeTangentSpace(CRadVertex& rVertex1, CRadVertex& rVertex2) const
{
...
void CLightScene::ComputeSmoothingInformation(RadSceneState& sceneState, const uint uiSmoothAngle, const bool cbTSGeneratedByLM) const
{
	...
	for (radpolyit iterPatchOuter = sceneState.lstScenePolys.begin(); iterPatchOuter != sceneState.lstScenePolys.end(); iterPatchOuter++)
	{
		//outer lightmap mesh loop
		CRadPoly* pPatchOuter = (*iterPatchOuter);
		
		//with each remaining one 
		for (radpolyit iterPatchInner = iterPatchOuter + 1; iterPatchInner != sceneState.lstScenePolys.end(); iterPatchInner++)
		{
...
float RadSceneState::ComputeHalvedLightmapQuality(const float fOldValue) const
{
	const float cfGridSize = genParam.m_fTexelSize;
	...
	const uint cuiSubSampleIndex) const
{
	...
	for (std::vector<LMCompLight*>::const_iterator ligIt = vLights.begin(); ligIt < vLights.end(); ligIt++)
	{
		...
		{
			UINT iNumHit = 0;
			Shadow(cLight, iNumHit, sceneState.pMesh, pSource, Vert, Vert.m_vPos);
	...
	{
		GLMOcclLightInfo& rOcclInfo = sceneState.pMesh->m_OcclInfo;//cache occlusion light info
		fSpotlightTerm = 0.f;
		for (std::vector<LMCompLight*>::const_iterator ligIt = vOcclLights.begin(); ligIt < vOcclLights.end(); ligIt++)
		{
			...
			if (eChannel == EOCCLCOLOURASSIGNMENT_NONE)
			{
				...
				char text[scuiWarningTextAllocation];
				sprintf(text, "GLM: %s has more than 4 active affecting occlusion lights\r\n", (const char*)sceneState.pMesh->m_sGLMName);
				sceneState.warningInfo.insert(std::make_pair((int)EWARNING_TOO_MANY_OCCL_LIGHTS, std::string(text)));
			}
	...
	if (cbFirstPass)
		...
		SetSubSampleDot3LightmapTexel(sceneState.subSampling, cuiSubSampleIndex, fColorRLamb, fColorGLamb, fColorBLamb, vLightDir, fLM_DP3LerpFactor, dot3Texel, occlTexel, m_sParam.m_bHDR);
}

void CLightScene::ProcessMesh(RadSceneState& sceneState) const
{
	CRadMesh* pMesh = sceneState.pMesh;
	IStatObj* pIGeom = sceneState.pIGeom;
	pMesh->PrepareLightmaps(sceneState, !m_sParam.m_bDontMergePolys, m_sParam.m_fTexelSize, sceneState.vMinBB, sceneState.vMaxBB);

	// No lightmap to calculate
	if (sceneState.lstScenePolys.empty())
		return;

	// set affecting lights for this patch
	const std::vector<LMCompLight*>& vLights = pMesh->m_LocalLights;
	const std::vector<LMCompLight*>& vOcclLights = pMesh->m_LocalOcclLights;

	//check normals
	if ((pMesh->m_uiFlags & DOUBLE_SIDED_MAT) != 0)
	{
		_TRACE(sceneState.logInfo, true, "GLM No.%i has double sided materials where lightmaps can't get applied properly...\r\n", sceneState.meshIdx);
		char text[scuiWarningTextAllocation];
		...
		sceneState.warningInfo.insert(std::make_pair((int)EWARNING_DOUBLE_SIDED, std::string(text)));
	}
	...
	if ((retValue = CheckNormals(fVariance, pMesh)) > 0)
	{
		...
		{
			_TRACE(sceneState.logInfo, true, "GLM No.%i has invalid normals, please do re-export, replacing normals...\r\n", sceneState.meshIdx);

			char text[scuiWarningTextAllocation];
			sprintf(text, "GLM: %s (%s) has invalid normals, please do re-export, normals replaced by default calculations\r\n", (const char*)pMesh->m_sGLMName, pIGeom ? pIGeom->GetFileName() : "");
			sceneState.warningInfo.insert(std::make_pair((int)EWARNING_WRONG_NORMALS, std::string(text)));
		}
		else if (retValue & NOT_NORMALIZED_NORMALS_FLAG)
		{
#ifdef DISPLAY_MORE_INFO
			_TRACE(sceneState.logInfo, true, "GLM No.%i has denormalized normals, max variance to 1.0: %f, renormalizing...\r\n", sceneState.meshIdx, fVariance);
#else	
			_TRACE(sceneState.logInfo, false, "GLM No.%i has denormalized normals, max variance to 1.0: %f, renormalizing...\r\n", sceneState.meshIdx, fVariance);
#endif
			char text[scuiWarningTextAllocation];
			...
			sceneState.warningInfo.insert(std::make_pair((int)EWARNING_DENORM_NORMALS, std::string(text)));
		}
	}
	...
	ComputeSmoothingInformation(sceneState, m_sParam.m_uiSmoothingAngle, bTSGeneratedByLM);

	//iterate all individual lightmap patches in this receiving GLM
	for (radpolyit i = sceneState.lstScenePolys.begin(); i != sceneState.lstScenePolys.end(); i++)
	{
		CRadPoly* pSource = (*i);
		...
		for (int y = 0; y < pSource->m_nH; y++)
			for (int x = 0; x < pSource->m_nW; x++)
			{
				...
				if (!pSource->InterpolateVertexAt((float)x, (float)y, Vert, dot3Texel, false, true))//snap middle point to ensure hit
					continue;
				DoLightCalculation(sceneState, pSource, uiMaxColourComp, vLights, vOcclLights, dot3Texel, Vert, true);
			} // x, y
			//now perform adaptive subsampling
#ifdef APPLY_COLOUR_FIX
		PerformAdaptiveSubSampling(sceneState, pSource, vLights, vOcclLights, uiMaxColourComp);
#else
		PerformAdaptiveSubSampling(sceneState, pSource, vLights, vOcclLights);
#endif
		//free some memory
...
bool CLightScene::Create(const IndoorBaseInterface& pInterface, const Vec3d& vMinBB, const Vec3d& vMaxBB, volatile SSharedLMEditorData* pSharedData, const ELMMode Mode, const uint cuiMeshesToProcessCount)
{
	...
	if (cuiMeshesToProcessCount > 0 && m_lstRadMeshes.size() > 0)
	{
		...
		{
			IStatObj* pIGeom = pMesh->m_pESource->GetEntityStatObj(0, NULL);
			int iNumIndices = 0;
	...
	std::vector<CRadMesh*> meshesToProcess;
	...
	for (; itRadMesh != m_lstRadMeshes.end(); itRadMesh++)
	{
		...
		RadSceneState sceneState(this->m_sParam, iCurMesh, pMesh, pIGeom, vMinBB, vMaxBB);

		if (m_sParam.m_iSubSampling > 1)
			InitSubSampling(sceneState.subSampling, m_sParam.m_iSubSampling);
		ProcessMesh(sceneState);

		m_LogInfo.insert(m_LogInfo.end(), sceneState.logInfo.begin(), sceneState.logInfo.end());
		m_WarningInfo.insert(sceneState.warningInfo.begin(), sceneState.warningInfo.end());

		// Create the lightmaps of this mesh
		if (!pMesh->SaveLightmaps(sceneState, m_pCurrentPatch, m_sParam.m_bDebugBorders))
		{
			...
			if (!pMesh->SaveLightmaps(sceneState, m_pCurrentPatch, m_sParam.m_bDebugBorders))
			{
				...
				if (pMesh->pLightmapQualityVar)
				{
					...
					{
						fCurrentValue = sceneState.ComputeHalvedLightmapQuality(fOldValue);//halve resolution
...
{
	subSampling.pfColours[4 * cuiSubSampleIndex] = fColorRLamb;
	subSampling.pfColours[4 * cuiSubSampleIndex + 1] = fColorGLamb;
	subSampling.pfColours[4 * cuiSubSampleIndex + 2] = fColorBLamb;
	subSampling.pfColours[4 * cuiSubSampleIndex + 3] = cfLM_DP3LerpFactor;
	rDot3Texel.vDot3Light = inLightDir;
	subSampling.pSubSamples[cuiSubSampleIndex] = rDot3Texel;
	if (subSampling.pOcclSamples)
		subSampling.pOcclSamples[cuiSubSampleIndex] = rOcclTexel;
}
...
#endif
{
	...
	*/
	...
	RadSubSampling& subSamling = sceneState.subSampling;

	if (subSamling.uiSampleCount > 1)//perform subsampling only if requested
	{
			...
			for (int y = cuiY * gAASettings.GetScale(); y < (cuiY + 1) * gAASettings.GetScale(); y++)//supersample
				for (int x = cuiX * gAASettings.GetScale(); x < (cuiX + 1) * gAASettings.GetScale(); x++)//supersample
				{
					...
					if (!pSource->InterpolateVertexAt((float)x, (float)y, Vert, dot3Texel, true, false))//do subsample and do not snap
						continue;
					subSamling.puiIndicator[uiSubSampleIndex] = 1;
					DoLightCalculation(sceneState, pSource, uiMaxColourComp, vLights, vOcclLights, dot3Texel,Vert, false, uiSubSampleIndex);
				} // x, y
				//apply subsamples
#ifdef APPLY_COLOUR_FIX
			pSource->GatherSubSamples(cuiX, cuiY, uiSubSampleIndex + 1, subSamling.puiIndicator, subSamling.pfColours, subSamling.pSubSamples, uiMaxComponent, subSamling.pOcclSamples, sceneState.pMesh->m_OcclInfo);
#else
			pSource->GatherSubSamples(cuiX, cuiY, uiSubSampleIndex + 1, subSamling.puiIndicator, subSamling.pfColours, subSamling.pSubSamples, m_pOcclSamples, sceneState.pMesh->m_OcclInfo);
#endif
		}
	}
	...
	SAFE_DELETE_ARRAY(pSource->m_pWSDominantDirData);
