...
struct RadSubSampling
{
	~RadSubSampling()
	{
		SAFE_DELETE_ARRAY(puiIndicator);
		SAFE_DELETE_ARRAY(pSubSamples);
		SAFE_DELETE_ARRAY(pOcclSamples);
...
struct RadSceneState
{
	...
	radpolylist				lstScenePolys;
	std::vector<CString>	logInfo;
	WarnInfoMap				warningInfo;

	float	ComputeHalvedLightmapQuality(const float fOldValue) const;
};
...
public:
	...
	void		PrepareLightmaps(RadSceneState& sceneState, bool bMergePolys, const float fGridSize, const Vec3d& vMinBB, const Vec3d& vMaxBB);
	bool		SaveLightmaps(RadSceneState& sceneState, CLightPatch* targetPatch, bool bDebug = false);
...
protected:
	...
	void PerformAdaptiveSubSampling(
		...
		CRadPoly* pSource,
		const std::vector<LMCompLight*>& vLights,
		const std::vector<LMCompLight*>& vOcclLights) const;
#endif
	...
	void InitSubSampling(RadSubSampling& subSampling, const uint cuiSampleCount) const
	{
		...
		if (m_sParam.m_bGenOcclMaps)
			subSampling.pOcclSamples = new SOcclusionMapTexel[cuiSampleCount];

		subSampling.uiSampleCount = cuiSampleCount;
	}
...
private:
	...
	void ProcessMesh(RadSceneState& sceneState) const;

	bool ComputeRasterCube(CRasterCubeImpl* pRasterCube, const std::set<IEntityRender*>& vGeom, const Matrix33* pDirLightTransf = NULL);
	void Shadow(LMCompLight& cLight, UINT& iNumHit, CRadMesh* pMesh, CRadPoly* pSource, const CRadVertex& Vert, const Vec3d& vSamplePos) const;
