...
void CRadMesh::PrepareLightmaps(RadSceneState& sceneState, bool bMergePolys, const float fGridSize, const Vec3d& vMinBB, const Vec3d& vMaxBB)
{
	...
	const LMGenParam& genParam = sceneState.genParam;
	...
	const int iMaxPolySize = genParam.m_iTextureResolution / 4;			// /4 is the maximum, but less size if better for packing
	for (radpolyit i1 = m_lstOldPolys.begin(); i1 != m_lstOldPolys.end(); i1++)
	{
...
#ifndef DISPLAY_MORE_INFO
		m_uiCoarseTexelCount += pNewPoly->CalcExtent(sceneState, m_sGLMName, s, false, fAlteredGridSize, MIN_LIGHTMAP_SIZE, iMaxPolySize, uiHugePatchFoundNumber);
#else
		m_uiCoarseTexelCount += pNewPoly->CalcExtent(sceneState, m_sGLMName, s, false, fAlteredGridSize, MIN_LIGHTMAP_SIZE, iMaxPolySize);
#endif
		...
		sceneState.lstScenePolys.push_back(pNewPoly);
	} //i  
#ifndef DISPLAY_MORE_INFO
	...
	if (((m_uiFlags & ONLY_SUNLIGHT) == 0) && (m_uiCoarseTexelCount > genParam.m_iTextureResolution * genParam.m_iTextureResolution))
	{
		...
		_TRACE(sceneState.logInfo, true, "GLM cannot fit into lightmap size %ix%i, adjusting values...\r\n",
			genParam.m_iTextureResolution, genParam.m_iTextureResolution);
		...
		if (pLightmapQualityVar)
		{
			...
			if (fCurrentValue > 0.f)//if not already reached maximum grid size
			{
				...
				while (uiIterations-- > 0)
				{
					if (uiIterations == 0)
						int michae = 0;
					fCurrentValue = sceneState.ComputeHalvedLightmapQuality(fOldCopiedValue);//halve resolution
			...
			sceneState.warningInfo.insert(std::make_pair((int)EWARNING_NO_FIT, std::string(text)));
		}
		//alter grid size and adapt individually
		float fAlteredGridSize = fGridSize;
		if (m_fLMAccuracy > 1.0f)
...
#endif
		for (auto radPoly : sceneState.lstScenePolys)
#ifndef DISPLAY_MORE_INFO
			radPoly->CalcExtent(sceneState, m_sGLMName, s, false, fAlteredGridSize, MIN_LIGHTMAP_SIZE, iMaxPolySize, uiHugePatchFoundNumber);
#else
			radPoly->CalcExtent(sceneState, m_sGLMName, s, false, fAlteredGridSize, MIN_LIGHTMAP_SIZE, iMaxPolySize);
#endif
#ifndef DISPLAY_MORE_INFO
		if (uiHugePatchFoundNumber == 1)
		{
			_TRACE(sceneState.logInfo, true, "Too huge patch found, lowering resolution...\r\n");
		}
		else if (uiHugePatchFoundNumber > 1)
		{
			_TRACE(sceneState.logInfo, true, "Too huge patches found, lowering resolution...\r\n");
		}
#endif
	}
}
...
//////////////////////////////////////////////////////////////////////		
bool CRadMesh::SaveLightmaps(RadSceneState& sceneState, CLightPatch* targetPatch, bool bDebug)
{
	// ---------------------------------------------------------------------------------------------
	...
	for (radpolyit i1 = sceneState.lstScenePolys.begin(); i1 != sceneState.lstScenePolys.end(); i1++)
	{
		if ((*i1) == NULL)
		...
		{
			delete pPoly; 
			*i1 = pPoly = NULL;//does only apply to patches
			continue;
		}
		...
		if (!targetPatch->GetSpace(nW, nH, pPoly->m_nOffsetW, pPoly->m_nOffsetH))
			return false;
	}
	...
	for (radpolyit i1 = sceneState.lstScenePolys.begin(); i1 != sceneState.lstScenePolys.end(); i1++)
	{
		...
		pPoly->CopyData(targetPatch, genParam.m_iTextureResolution);

		const double cdInvTexRes = 1.0 / (double)(genParam.m_iTextureResolution);
...
#endif
{
	...
	if (m_nW > iMaxBlockSize || m_nH > iMaxBlockSize)
	{
		...
		if (m_nW > iMaxBlockSize && m_nH > iMaxBlockSize)
		{
			...
			_TRACE(sceneState.logInfo, true, "Huge patch found: %ix%i, allowed maximum: %ix%i texels (lowering resolution...)\r\n", iOldW, iOldH, iMaxBlockSize, iMaxBlockSize);
#else
			_TRACE(sceneState.logInfo, false, "Huge patch found: %ix%i, allowed maximum: %ix%i texels (lowering resolution...)\r\n", iOldW, iOldH, iMaxBlockSize, iMaxBlockSize);
#endif
			char text[scuiWarningTextAllocation];
			...
			sceneState.warningInfo.insert(std::make_pair((int)EWARNING_HUGE_PATCH, std::string(text)));
		}
		else
		{
